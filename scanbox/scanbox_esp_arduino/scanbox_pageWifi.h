void handleMenuWifi(AsyncWebServerRequest *request)
{
  ...
  response += "<title>Paramètres WiFi</title>";
  response += "<style>";
  ...
  response += "  </div>";
  response += "  <div class='form-group'>";
  response += "    <input type='checkbox' id='toggleWifiPassword' onchange='toggleWifiPasswordField()' " + String((wifiPassword.length() > 0 ? "checked" : "")) + "> Mot de passe requis";
  response += "  </div>";
  response += "  <div class='form-group'>";
  response += "    <label for='wifiPassword'>Mot de passe:</label>";
  response += "    <input type='password' id='wifiPassword' maxlength='50' " + String((wifiPassword.length() > 0 ? "placeholder='********'" : "disabled")) + ">";
  response += "  </div>";
  response += "  <button onclick='saveWifiSettings()'>Enregistrer</button>";
  response += "</div>";
  response += "<a href='/'>Retour</a>";
  response += "<div class='info'>";
  response += "  <b>Adresse IP:</b> " + (WiFi.getMode() != WIFI_AP ? WiFi.localIP().toString() : WiFi.softAPIP().toString()) + "<br>";
  ...
  response += "  <b>Mode WiFi:</b> " + String((WiFi.getMode() != WIFI_AP ? "Station" : "Point d'accès")) + "<br>";
  response += "</div>";
  response += "</body>";
  response += "</html>";
