...
camerasStore.getCameraByInterval();
            ...
            <template v-if="!camerasStore.loading">
                <tr v-for="camera in camerasStore.cameras" :key="camera.id_camera">
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">
                        <button @click="openEditModal(camera)"
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">
                        <button @click="camerasStore.toggleLight(camera)"
                            class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">
                            toggle
                        </button>
                    </td>
                    <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">
                        <div v-if="!camerasStore.stream[camera.id_camera]">
                            <button @click="camerasStore.getStream(camera.id_camera)"
                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">
                                enable
                            </button>
                        </div>
                        <div v-else>
                            <button @click="camerasStore.stopStream(camera.id_camera)"
                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">
                                disable
                            </button>
                        </div>
