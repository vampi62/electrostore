...
async function fetchAllData() {
	if (cameraId.value === "new") {
		camerasStore.loadToEdition(cameraId.value, preset.value);
	} else {
		camerasStore.setLoadingEdition(cameraId.value, true);
		try {
		...
		camerasStore.loadToEdition(cameraId.value);
	}
}
onMounted(() => {
	fetchAllData();
...
onBeforeUnmount(() => {
	...
	camerasStore.clearEdition(cameraId.value);
});
...
const cameraSave = async() => {
	...
	try {
		...
		if (!allValid) {
			...
			camerasStore.setLoadingEdition(cameraId.value, false);
			return;
		}
		if (cameraId.value === "new") {
			const newId = await camerasStore.createCamera({ ...camerasStore.cameraEdition[cameraId.value] } );
			camerasStore.loadToEdition(newId);
			addNotification({ message: t("camera.Created"), type: "success" });
			cameraId.value = String(newId);
			router.push("/cameras/" + cameraId.value);
		} else {
			await camerasStore.updateCamera(cameraId.value, { ...camerasStore.cameraEdition[cameraId.value] });
			camerasStore.loadToEdition(cameraId.value);
			camerasStore.getStatus(cameraId.value);
			camerasStore.getStream(cameraId.value);
...
const createSchema = () => {
	const edition = camerasStore.cameraEdition[cameraId.value];
	const shape = {};
	if (!edition) {
...
<template>
	<div class="flex items-center justify-between mb-4">
		...
		<TopButtonEditElement
			:main-config="{ path: '/cameras',
				create: { showCondition: cameraId === 'new' && authStore.hasPermission([2]), loading: camerasStore.cameraEdition[cameraId]?.loading },
				update: { showCondition: cameraId !== 'new' && authStore.hasPermission([2]), loading: camerasStore.cameraEdition[cameraId]?.loading },
	...
	<div v-if="camerasStore.cameras[cameraId] || cameraId == 'new'" class="w-full">
		<div class="mb-6 flex justify-between flex-wrap w-full gap-y-4 gap-x-4">
			<FormContainer ref="formContainer" :schema-builder="createSchema" :labels="labelForm" :store-data="camerasStore.cameraEdition[cameraId]" :store-user="authStore.user"
