...
function selectLed(led, event = null) {
    if (event) { showEditMenu(event) } // if right click show the menu
    saveMouseClickPos()
    if (selectedElement.value.key == led) { return } // if click on the element already selected don't do anything
...
function selectBox(box, event = null) {
    if (event) { showEditMenu(event) } // if right click show the menu
    saveMouseClickPos()
    if (selectedElement.value.key == box) { return } // if click on the element already selected don't do anything
    stopSelecting() // unselect if another element is selected
