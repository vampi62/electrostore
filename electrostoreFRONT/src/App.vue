...
const listNav = [
    ...
    { name: 'tags', path: '/tags', roleRequired: 'user', faIcon: 'fa-solid fa-tags' },
    { name: 'stores', path: '/stores', roleRequired: 'user', faIcon: 'fa-solid fa-store' }
];
</script>
...
<template>
    <div class="app-container bg-gray-100">
        <nav v-show="authStore.user"
            class="flex justify-between p-5 bg-gray-800 border-b-2 border-blue-400 fixed w-full top-0 h-16">
        ...
        <div v-show="authStore.user" :class="['hidden sm:flex flex-col justify-between p-4 bg-gray-800 fixed left-0 top-16 bottom-12',
        ...
        <div :class="['p-4 overflow-y-scroll fixed bottom-0 top-16 right-0 left-0',
            reduceLeftSideBar ? 'sm:ml-16' : 'sm:ml-64']">
            <RouterView />
        </div>
    </div>
</template>