...
namespace electrostore.Controllers
{
    ...
    public class CommandDocumentController : ControllerBase
    {
        ...
        [FromQuery, SwaggerParameter(Description = "(Optional) RSQL string to filter results. Example: 'name_command_document=like=example'.")] string? filter = null,
        [FromQuery, SwaggerParameter(Description = "(Optional) Sort string to order results. Example: 'name_command_document,asc' or 'name_command_document,desc'.")] string? sort = null)
        {
            var rsqlDto = ParserExtensions.ParseFilter(filter ?? string.Empty);
            var sortDto = ParserExtensions.ParseSort(sort ?? string.Empty);
            var commandsDocuments = await _commandDocumentService.GetCommandsDocumentsByCommandId(id_command, limit, offset, rsqlDto, sortDto);
