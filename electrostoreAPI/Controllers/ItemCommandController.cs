...
namespace electrostore.Controllers
{
    ...
    public class ItemCommandController : ControllerBase
    {
        ...
        public async Task<ActionResult<PaginatedResponseDto<ReadExtendedCommandItemDto>>> GetCommandsItemsByItemId([FromRoute] int id_item, [FromQuery] int limit = 100, [FromQuery] int offset = 0,
        ...
        [FromQuery, SwaggerParameter(Description = "(Optional) Sort string to order results. Example: 'prix_command_item,asc' or 'prix_command_item,desc'.")] string? sort = null)
        {
            ...
            var commandItems = await _commandItemService.GetCommandsItemsByItemId(id_item, limit, offset, rsqlDto, sortDto, expand);
            return Ok(commandItems);
        }

        [HttpGet("{id_command}")]
        [Authorize(Policy = "AccessToken")]
        public async Task<ActionResult<ReadExtendedCommandItemDto>> GetCommandItemById([FromRoute] int id_item, [FromRoute] int id_command,
        [FromQuery, SwaggerParameter(Description = "(Optional) Fields to expand. Possible values: 'command', 'item'. Multiple values can be specified by separating them with ','.")] List<string>? expand = null)
        {
