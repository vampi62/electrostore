...
public class ProjetCommentaireService : IProjetCommentaireService
{
    ...
    List<FilterDto>? rsql = null, SorterDto? sort = null, List<string>? expand = null)
    {
        ...
        {
            var sortResult = RsqlParserExtensions.ToSortExpression<ProjetsCommentaires>(sort);
    ...
    List<FilterDto>? rsql = null, SorterDto? sort = null, List<string>? expand = null)
    {
        ...
        return new PaginatedResponseDto<ReadExtendedProjetCommentaireDto>
        {
            ...
            pagination = new PaginationDto
            {
                offset = offset,
                limit = limit,
                total = await _context.ProjetsCommentaires.CountAsync(filterResult ?? (pc => pc.id_user == userId)),
                nextOffset = offset + limit,
                hasMore = await _context.ProjetsCommentaires.Skip(offset + limit).AnyAsync(filterResult ?? (pc => pc.id_user == userId))
            },
            filters = rsql,
            sort = sort != null ? [sort] : null
        };
